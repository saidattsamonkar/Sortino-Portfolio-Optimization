...
# Months to look back while computing IR weights
LOOKBACK_MONTHS = 12
LOOKBACK_MONTHS_2 = 0
...
class PortfolioOptimization():
    ...
    def Load_data(self):
        ...
        # retreiving Equities as a matrix
    ...
    def Opt_IR(self, p, b):
        ...
        '''exps = np.array(range(1, len(p) + 1))
        for i in range(0,len(exps)):
            exps[i]= (1.02)**exps[i]'''

        def opt(w):
            #IR
            #return -((np.sum((np.subtract(np.sum(p * w, axis=1), b).T * exps) / np.sum(exps))) / (np.std(np.subtract(np.sum(p * w, axis=1), b))))
            ###return -( np.mean(np.subtract(np.sum(p * w, axis=1), b)) / (np.std(np.subtract(np.sum(p * w, axis=1), b))))
            return -(np.mean(np.subtract(np.sum(p * w, axis=1), b)) / (np.std(np.subtract(   np.sum(  ((p * w)-np.abs(p*w))/2  , axis=1)   , (b-np.abs(b))/2 ))))
        ...
        if self.FirstIter == False:
            ...
            print("MC")
            print((np.sum(np.abs(self.OldWeights - res.x)) / 2))
    ...
    def Opt_SR(self, p, b):
        ...
        def opt(w):
            '''n = len(b)
            ranked = np.sort(np.sum(p * w, axis=1))
            ranks = list(range(1 , n+1 ))
            covar = (np.cov(ranked,ranks)[0][1])
            mean = np.mean(np.sum(p * w, axis=1))

            return ((2*covar)/mean*n)
            '''
            #for PSR
            '''n=len(b)

            rets = np.sum(p * w, axis=1)
            skw = skew(rets,bias=True)
            kurt = kurtosis(rets,bias=True)
            sharpe_p = np.mean(rets)/np.std((rets-np.abs(rets))/2)
            sharpe_b = np.mean(b)/np.std((b-np.abs(b))/2)

            sharpe_std = np.sqrt( ((1-skw)*sharpe_p+((kurt+2)/4)*(sharpe_p**2))/(n-1) )

            return -norm.cdf((sharpe_p-sharpe_b)/sharpe_std)
            '''
            ...
            #Sharpe
            #return -((np.sum(((np.sum(p * w, axis=1)).T * exps) / np.sum(exps))) / (np.std((np.sum(p * w, axis=1)))))

            return -(np.mean(np.sum(p * w, axis=1))/np.std(np.sum(px * w, axis=1)))
    ...
    def Opt_EF(self, p, b):
        ...
        covMatrix = np.array(pd.DataFrame.cov(p).values).astype(np.float)
        ...
        def opt(w):
            return (w @ covMatrix @ np.transpose(w))
    ...
    def Opt_IR_Final(self, p, b,wf):

        rets=[]
        p = np.array(p.values).astype(np.float) + 1


        for wff in wf:
            pp=p.copy()
            pp[0] = np.multiply(wff, pp[0])
            for i in range(1, len(pp)):
                pp[i] = np.multiply(pp[i - 1], pp[i])
            pp = np.sum(pp, axis=1)
            rets+=[pp]

        #print(np.shape(np.array(rets)))

        covMatrix = np.cov(np.array(rets))

        w0 = np.array([0.1] * len(wf))

        bnds = ([0.0,1.0],[0.0,1.0],[0.0,1.0])


        def opt(w):
            return (w @ covMatrix @ w.T)
    ...
    def Plot(self, w, p, b):
        ...
        print('IR:')
        print((np.mean((np.subtract(np.sum(p * w, axis=1), b))) / np.std(np.subtract(np.sum(p * w, axis=1), b))))




        self.IRs += [
            ((np.mean((np.subtract(np.sum(p * w, axis=1), b))) / np.std(np.subtract(np.sum(p * w, axis=1), b))))]
        ...
        print ( (np.mean(pp)-np.mean(bb)) /np.std(pp))
        print( (np.mean(pp)) /np.std(pp))
        print( (np.mean(bb)) /np.std(bb))
        #self.SPs += [( (np.mean(pp)) /np.std(pp))]
        #self.SPs2 += [( (np.mean(bb)) /np.std(bb))]
        self.SPs += [(p[-1]-p[0])/p[0]]
        self.SPs2 += [(b[-1]-b[0])/b[0]]
        ...
        if self.FirstIter == True:
            ...
            #print(np.sum(self.OldWeights - ow))
    ...
    def run(self):
        # array, benchmark, dates = self.load_data()
        ...
        for month in tqdm(range(LOOKBACK_MONTHS, len(pdf))):
            pdf_ = pd.concat([pdf[x] for x in range(month - LOOKBACK_MONTHS, month - LOOKBACK_MONTHS_2)],
                             ignore_index=True, axis=0)
            bdf_ = pd.concat([bdf[x] for x in range(month - LOOKBACK_MONTHS, month - LOOKBACK_MONTHS_2)],
                             ignore_index=True, axis=0)
            # pdf_=pdf[month]
            # bdf_ = bdf[month]
            ...
            '''
            w1 = self.Opt_IR(pdf_, bdf_)
            w2 = self.Opt_SR(pdf_, bdf_)
            w3 = self.Opt_EF(pdf_, bdf_)

            wf = self.Opt_IR_Final(pdf_,bdf_,[w1,w2,w3])

            self.NewWeights = np.add(np.add(w1*wf[0],w2*wf[1]),w3*wf[2])
            '''
            ...
            self.Plot(self.NewWeights, pdf[month], bdf[month])
        ...
        print('SHARPE')
        print("PORTFOLIO: ", end=' ')
        print((np.mean(self.SPs) / np.std(self.SPs)) * math.sqrt(12))
        print("BENCHMARK: ", end=' ')
        print((np.mean(self.SPs2) / np.std(self.SPs2)) * math.sqrt(12))
        print('SORTINO')
        print("PORTFOLIO: ", end=' ')
        ...
        plt.plot()
        #plt.savefig(str(startDateA) + '__' + str(endDateA) + '__ALL__OPT.png')
        plt.savefig('SR.png')
        plt.show()
...
if __name__ == "__main__":
    ...
    POPT.Download_data(startDate=startDateA, endDate=endDateA, overwrite=True)
    POPT.run()
